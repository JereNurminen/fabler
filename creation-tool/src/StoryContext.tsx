...
export const StoryProvider: React.FC<StoryContextProps> = ({ children }) => {
  ...
  const loadStory = useCallback(async (id: number) => {
    ...
    handleResult(
      storyResult,
      (ok) => {
        ok.data.pages.forEach((page) =>
          pageMap.set(page.id, {
            status: "loaded",
            value: { status: "ok", data: page },
          }),
        );
        setStory({ status: "loaded", value: { status: "ok", data: ok.data } });
      },
      (err) => {
        setStory({ status: "loaded", value: err });
      },
    );
  }, []);
  ...
  const getPage = useCallback(async (pageId: number) => {
    ...
    return handleResult<Page, Loadable<Page>>(
      pageResult,
      (ok) => {
  ...
  return (
    <StoryContext.Provider
      value={{
        story,
        pages: Array.from(pageMap.map.values()),
        loadStory,
        getPage,
        patchPage,
        createPage,
